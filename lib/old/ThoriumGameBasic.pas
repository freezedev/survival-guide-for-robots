...
type
  ...
  TGUILayer = class (TThoriumPersistent)
  public
    constructor Create;
  protected
    class function Construct(): TGUILayer;
    class procedure GetMethodList(Sender: TThoriumRTTIObjectType;
       var Methods: TThoriumRTTIMethods); override;
    class procedure GetStaticMethodList(Sender: TThoriumRTTIObjectType;
       var Methods: TThoriumRTTIStaticMethods); override;
  public
    procedure Box(aX, aY, aW, aH: Integer; aR, aG, aB, aA: Byte);
  end;
...
class function TGUILayer.Construct(): TGUILayer;
// A note about this function. It is *neccessary* to use a static (class) method
// instead of the direct call to a constructor. This is due to pascal internas
// which seem to happen at constructor call. Maybe there will be an alternative
// for this later. A special type for constructors for example ;).
begin
  ...
  // for the script only, you would call the constructor and get a new
  // reference:
  Result := TGUILayer(TGUILayer.Create.ThoriumReference.GetReference);
  // And disable the host control.
  Result.ThoriumReference.DisableHostControl;
end;

class procedure TGUILayer.GetMethodList(Sender: TThoriumRTTIObjectType;
  var Methods: TThoriumRTTIMethods);
begin
  SetLength(Methods, 1);
  ...
  Methods[0] := TThoriumRTTIObjectType.NewNativeCallMethod(
    'box',
    @TGUILayer.Box,
    [htLongInt, htLongInt, htLongInt, htLongInt, htByte, htByte, htByte, htByte],
    htNone,
    ncRegister
  );
end;

class procedure TGUILayer.GetStaticMethodList(Sender: TThoriumRTTIObjectType;
  var Methods: TThoriumRTTIStaticMethods);
begin
  SetLength(Methods, 2);
  Methods[0] := TThoriumRTTIObjectType.NewNativeCallStaticMethod(
    'create',
    @TGUILayer.Construct, TGUILayer,
    [],
    htExt,
    ncRegister
  );
...
procedure TGUILayer.Box(aX, aY, aW, aH: Integer; aR, aG, aB, aA: Byte); inline;
begin
  GUI.Box(makeRect(aX, aY, aW, aH), makeCol(aR, aG, aB, aA));
end;
...
var
  ...
  ValueApp, ValueInput, ValueGUI: TThoriumValue;
  AppObject, InputObject, GUIObject: TThoriumRTTIObjectType;
begin
  ...
  GUIObject := RegisterRTTIType(TGUILayer, False);

  TypeSpecInput.ValueType := vtExtendedType;
  TypeSpecInput.Extended := GUIObject;
  ValueGUI := ThoriumCreateExtendedTypeValue(GUIObject);

  ValueGUI.Extended.Value := GUILayer;
  RegisterPropertyDirect('input', TypeSpecInput, True).Value := ThoriumDuplicateValue(ValueGUI);
