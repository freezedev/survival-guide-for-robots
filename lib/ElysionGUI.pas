...
TelMenuLayout = (mlVertical, mlHorizontal);
 ...
 public
   ...
   function OnButtonClick(aID: Integer): Boolean; Overload; {$IFDEF CAN_INLINE} inline; {$ENDIF}
   function OnButtonClick(Caption: String): Boolean; Overload; {$IFDEF CAN_INLINE} inline; {$ENDIF}  // Maybe improve => Runtime: O(n)

   function OnButtonMouseOver(aID: Integer): Boolean; Overload; {$IFDEF CAN_INLINE} inline; {$ENDIF}
   function OnButtonMouseOver(Caption: String): Boolean; Overload; {$IFDEF CAN_INLINE} inline; {$ENDIF} // Maybe improve => Runtime: O(n)
...
function TelLabel.GetOnMouseOver(): Boolean;
begin
...
function TelLabel.GetOnClick(): Boolean;
begin
  if (Self.OnMouseOver and Input.Mouse.LeftClick) then
    Result := true
  else Result := false;
end;
...
function TelButton.GetOnMouseOver: Boolean;
var
  fRect: TelRect;
begin
  fRect := makeRect(Position.X, Position.Y, GetWidth(), GetHeight());
...
function TelButton.GetOnClick: Boolean;
begin
...
begin
  if OnMouseOver and Input.Mouse.RightClick then Result := true
  else Result := false;
end;
...
function TelMenu.GetOnMouseOver(): Boolean;
var
  i: Integer;
begin
  for i := 0 to fButtonList.Count - 1 do
  begin
    if TelButton(fButtonList.Items[i]).OnMouseOver then
    begin
...
function TelMenu.GetOnClick(): Boolean;
var
  i: Integer;
begin
  for i := 0 to fButtonList.Count - 1 do
  begin
    if TelButton(fButtonList.Items[i]).OnClick then
    begin
      fCurrentButton := TelButton(fButtonList.Items[i]);
      Result := true;
      Exit;
...
function TelMenu.OnButtonClick(aID: Integer): Boolean;
begin
  Result := (TelButton(fButtonList.Items[aID])).OnClick;
end;

function TelMenu.OnButtonClick(Caption: String): Boolean;
var
  i: Integer;
begin
  for i := 0 to fButtonList.Count - 1 do
  begin
    ...
    begin
      Result := (TelButton(fButtonList.Items[i])).OnClick;
      Exit;
    end;
  end;
end;

function TelMenu.OnButtonMouseOver(aID: Integer): Boolean;
begin
  Result := (TelButton(fButtonList.Items[aID])).OnMouseOver;
end;

function TelMenu.OnButtonMouseOver(Caption: String): Boolean;
var
  i: Integer;
begin
  for i := 0 to fButtonList.Count - 1 do
  begin
    if (TelButton(fButtonList.Items[i]).Caption = Caption) then
    begin
      Result := (TelButton(fButtonList.Items[i])).OnMouseOver;
      Exit;
    end;
  end;
end;
