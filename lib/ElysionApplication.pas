...
TelWindow = class(TelObject)
  private
    ...
    fLimitFPS: Boolean;
    fFrameCap, fFPS: Single;

    fFPSTimer, fDeltaTimer: TelTimer;
  ...
  function SetVideoMode(_Width, _Height, _ColorBits: Integer; _Fullscreen: Boolean; _VideoFlag: TelVideoFlag): Boolean; Overload;
	...
	function SetVideoMode(_Width, _Height, _ColorBits: Integer; _Fullscreen: Boolean = False): Boolean; Overload; {$IFDEF CAN_INLINE} inline; {$ENDIF}
        ...
        procedure SetNativeResolution(Width, Height: Integer); {$IFDEF CAN_INLINE} inline; {$ENDIF}
...
begin
  ...
  // DEBUG YOU! (I don't think anyone except me understand these comments :) )
  Self.Log('Opened Joystick 0');
  Self.Log('Joystick Name: ' + SDL_JoystickName(0));
  Self.Log('Number of Axis: ' + IntToStr(SDL_JoystickNumAxes(fJoystick)));
  Self.Log('Number of Buttons: ' + IntToStr(SDL_JoystickNumButtons(fJoystick)));
  Self.Log('Number of Balls: ' + IntToStr(SDL_JoystickNumBalls(fJoystick)));
...
begin
  ...
  if ((FNativeResolution.X <> 0) and (FNativeResolution.Y <> 0)) then
    Self.SetNativeResolution(FNativeResolution.X, FNativeResolution.Y);
end;

procedure TelWindow.SetNativeResolution(Width, Height: Integer); 
begin
  if (FNativeResolution.X <> Width) then FNativeResolution.X := Width;
  if (FNativeResolution.Y <> Height) then FNativeResolution.Y := Height;

  // Set resolution scale
  fResScale.X := FResolution.X / Width;
  fResScale.Y := FResolution.Y / Height;


  if isLoggerActive then TelLogger.GetInstance.WriteLog(Format('Resolution: %d x %d pixels', [FResolution.X, FResolution.Y]), ltNote);
  if isLoggerActive then TelLogger.GetInstance.WriteLog(Format('Native Resolution: %d x %d pixels', [FNativeResolution.X, FNativeResolution.Y]), ltNote);
  if isLoggerActive then TelLogger.GetInstance.WriteLog(Format('Scale.X: %f  Scale.Y: %f', [ResScale.X, ResScale.Y]), ltNote);
end;
...
begin
  if (fFPSTimer.GetTicks() >= 1000) then
  begin
    fFPS := Trunc((fFrames / (fDeltaTimer.GetTicks() / 1000)) * 100) div 100;
  ...
  (*if ((fFPSTimer.getTicks > 0) and (fFrames > 0)) then
  begin
    Result := fFrames / ( fFPSTimer.getTicks() / 1000 );
  end else Result := 0; *)
end;
...
begin
  if fDeltaTime > 0 then Result := fDeltaTime
    else Result := 0.001; // < Be careful - Might not get expected results all the time
end;
...
function TelWindow.GetTicks(): Integer;
begin
  Result := SDL_GetTicks();
end;
