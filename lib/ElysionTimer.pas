...
TelTimer = class(TelObject, ITimer)
  private
    ...
    function GetOnEvent(): Boolean; {$IFDEF CAN_INLINE} inline; {$ENDIF}
  public
  ...
  published
    ...
    property Event: TelEvent read fEvent write fEvent;
    ...
    // In that case use the GetTicks() - function or SDL_AddTimer
...
begin
  if Self.OnEvent then Event();
end;

function TelTimer.GetOnEvent: Boolean; {$IFDEF CAN_INLINE} inline; {$ENDIF}
begin
  Result := false;
