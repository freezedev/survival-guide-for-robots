...
TelMouseHelper = class
  ...
  public
    ...
    function Motion(Point: TelVector2i): Boolean; Overload; {$IFDEF CAN_INLINE} inline; {$ENDIF}
    function Motion(Rect: TelRect): Boolean; Overload; {$IFDEF CAN_INLINE} inline; {$ENDIF}

    //function GetLastPosition: TelVector2i;
end;
...
begin
  Result := isButtonHit(BUTTON_LEFT);
end;

function TelMouseHelper.RightClick: Boolean; 
begin
  Result := isButtonHit(BUTTON_RIGHT);
end;
...
begin
  fLastCursorPos := ActiveWindow.Cursor;
end;

function TelMouseHelper.Motion(Point: TelVector2i): Boolean; 
begin
